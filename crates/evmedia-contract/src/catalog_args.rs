...
/// Which part of an EVS export to run. The GUI pipelines download, merge and publish separately.
#[derive(clap::ValueEnum, Debug, Clone, Copy, PartialEq, Eq)]
pub enum ExportPhase {
    All,
    Download,
    /// Backwards-compatible local operation: merge then publish one cached lesson.
    Convert,
    /// Derive, decrypt and merge cached segments into `lesson.ts`.
    Merge,
    /// Validate and remux the already merged `lesson.ts` into the requested container.
    Publish,
}
...
pub struct ExportEvsArgs {
    ...
    #[arg(long)]
    pub force: bool,
    /// Run the complete export, or one pipeline stage over this lesson's cached work directory.
...
impl ToArgv for ExportEvsArgs {
    fn to_argv(&self) -> Vec<String> {
        ...
        argv.extend([
            "--phase".into(),
            self.phase.to_possible_value().unwrap().get_name().into(),
        ]);
        argv
    }
}
...
impl ToArgv for ExportBatchArgs {
    fn to_argv(&self) -> Vec<String> {
        let mut argv = vec![
            ...
            self.account.to_string(),
            "--plan".into(),
            self.plan.to_string_lossy().into_owned(),
            "--download-jobs".into(),
            self.download_jobs.to_string(),
            "--decrypt-jobs".into(),
            self.decrypt_jobs.to_string(),
            "--ffmpeg".into(),
            self.ffmpeg.clone(),
            "--ffprobe".into(),
            self.ffprobe.clone(),
