...
pub async fn run(args: ExportBatchArgs, reporter: &Reporter) -> Result<()> {
    ...
    for p in plan.videos {
        ...
        match prepare(&api, args.account, p).await {
            Ok((owner, lesson_tasks, lesson)) => { tasks.extend(lesson_tasks); lessons.insert(owner, lesson); }
            Err(error) => { reporter.info(format!("batch preparation failed: {error}")); failures.push(error.to_string()); }
        }
    }
...
async fn prepare(api: &CatalogApi, account: i64, p: ExportBatchVideo) -> Result<(String, Vec<segment_queue::SegmentTask>, Lesson)> {
        ...
        Ok((owner, tasks, Lesson { output: p.output, work: p.work, vod, total, next: 0, received: 0, pending: BTreeMap::new(), partial }))
}
