...
1. **在线重取清单**：为盘上每节课重新取一次 `k_l`。需要三样东西——有效的 JWT
   （`tools/parser-tools/captured/tokens.jsonl` 里的 JWT 带 `exp`，写文档时已可能过期，用前先解出来看）、
   该课的 `cache_key`（= 签名前像里的 `evs_playkey`）、以及**该课完整的分段文件名列表**。
   前两样都有线索（`Admin.conf` 提供课程 uuid 与 lessonId；`cache_key` 在 descriptor 里出现过），
   **第三样来源未确认**——播放器是先知道文件名再发 `ts_liststr=0|0|<名字>,1|0|<名字>,…` 的，
   这些名字来自哪个响应，本项目没有查明。
