...
| 播放器 DLL 里没有标准 CABAC 表 | spec 的 `rangeTabLPS`（FFmpeg 的 `lps_range`，第 0–8 行）在 DLL 里整份搜不到；两份 stock 二进制里都在 | `find_tables.py`、`probe_tables.py` |
| 运行中的播放器里，标准表只在显卡驱动里 | 119 个已加载模块 + 544 段私有内存（472 MB）中只有一处命中：NVIDIA 的 `nvd3dumx.dll` `0x7ffe71d47230` | `find_stock_cabac.py` |

**HYPOTHESIS（假设）——只是提出，尚未验证。**

1. level 40 这一类分段受一层保护，而它住在播放器自己的熵解码器里：载荷看起来是普通带转义的 H.264，
   没有任何 stock 解码器能读，而播放器的 DLL 缺标准 CABAC 表、又带着一份布局不同的表。
   这是目前唯一同时装得下 §14.4–§14.10 全部观测的说法，但它是**按排除法得出的定性结论**，
   机制本身没有被验证：没有把 DLL 的表与 stock 的表逐项比出「改掉了哪些数」，
   也没有证明改过的表就是 level 40 解不出来的原因。
2. 播放器确实渲染了这些 level 40 分段。这仍然是**间接推断**，不是实测：播放器在播放期间下载了它们，
   使用者报告这节课能播。把它变成实测的办法是 §14.12 那个注入实验，重跑一次。
...
stock 表附近那十三个 32 字节窗口，在 DLL 里同样一个都不出现。
也就是说，不只是表本身不在，表所在的那片区域也不在。

工具：`tools/parser-tools/find_stock_cabac.py`。它扫的是**运行中的**播放器：全部 119 个已加载模块，
加上 544 段私有内存（472 MB）。标准表在整个进程里只存在一处——NVIDIA 的 `nvd3dumx.dll` 里
`0x7ffe71d47230`；另外有一个堆块含有 `lps_range` 第 4 行四次。

两处都要读准：

* `nvd3dumx.dll` 是显卡驱动自己的东西，不是播放器的模块。而 §14.7 里走 NVIDIA 硬解的 `h264_cuvid`
  同样是平灰——所以这一份命中解释不了播放器的行为。
* 那个堆块（`lps_range` 第 4 行 ×4）**没有解释**。记在这里是为了它不被忘掉，不是把它当线索。

结论：**播放器自己的软件解码器，在进程里的任何地方都不带标准 CABAC 表。**
这里的「没有」是「在这 119 个模块与 544 段私有内存里、按这个字节指纹没找到」：
字节相同但换了位置的表会被找到，所以没找到只有两种解释——表不存在，或者表的字节被改过。
本节区分不了这两者；§14.10 的布局差异指向后者，但那是指向，不是证明。
...
站得住的结论：**level 40 这一类分段受一层保护，而它住在播放器自己的熵解码器里**——
载荷看起来是普通带转义的 H.264，没有任何 stock 解码器（八种实现）能读，
而播放器的 DLL 缺标准 CABAC 表、又带着一份布局不同的表。这是按排除法得到的定性结论，
机制本身未验证（§14.1 假设 1）。

仍然未决的那个问题：**播放器是否真的渲染了这些 level 40 分段。**
这仍然是间接推断——播放器在播放期间下载了它们，使用者报告这节课能播——不是实测。
把它变成实测的办法就是 §14.12 那个实验，在窗口真的在屏幕上时重跑。
