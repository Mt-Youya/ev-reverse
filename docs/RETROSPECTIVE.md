...
## 15. 2026-09-16/17 会话：把"解密任意一节课"做成工具，并把第二层钉死在代码里

### 15.1 工具（现在是一条命令）
lesson_pipeline.py：循环跑 harvest（只读钩子收播放器的 key）→ map_keys.py（把 key 映射到文件；掩码只依赖文件名，所以掩码一次、每把 key 只跑 AES）→ key_inventory.py（挖抓到的课程列表、逐把 key 验证、报库容量）→ decrypt_lesson.py（解密、把每个段放到同一条时间线、合并、**逐段量时长**）。
whats_playing.py：不用问人就知道现在在播哪一节、哪个位置、属于哪一类。

### 15.2 时长这件事上，实测抓到的真问题
* 同一段 10 秒在服务器上有多个编码变体（不同 uuid、pts 网格偏移几秒）。混着合并 → 视频能播但长 20–40%。
* 字节拼接 + -c copy 保留各段原始 PTS → 时长变成"偏移的跨度"。改用 ffmpeg concat（重写时间戳）后，时长 = 内容长度。
* 	k 属于播放会话：用 A 会话列表的 tk 去解 B 会话下载的文件必然错（实测 190/742 解出来同步率 0.5% = 随机）。**钥匙必须逐把验证后才能算覆盖**。
* 位置要靠"内容位置"（列表的 idx）或"会话零点"（同一会话内 pts − idx×10s 是常数）来定；网格必须以定义它的会话长度为界，否则会凭空造出位置（曾报出 287 而列表只有 195 条）。

### 15.3 第二层：不在数据里，在代码里（本轮最重要的实测）
* 45 秒 1001 次调用中，带 level-40 参数集的 4 次调用**全部返回真画面**（帧方差 27.8–29.1）。同一函数、同一批字节，八种原版解码器却整帧掩盖。
* 播放器 DLL 里**所有 H.264 表都是原版的**：算术表（lps_range / 状态转移 / 重归一化）是规范那套数、只是按 QP 主序重复存放；CABAC 上下文初始化表**整张 2048 字节与原版逐字节相同**。
* 已排除：解码器版本（4.2.2 同样灰）、参数集是诱饵（换 level-42 的 SPS/PPS 无效）、PPS 谎报熵模式（改成 CAVLC 错误更多）、表被改。
* 结论：第二层是**解码器代码路径**上的东西（或流本身与表象不同），下一步是把该函数与其 stock 对应物做代码级对比。

### 15.4 当前边界
缓存 11,788 个分段、钥匙库 583 把（全部验证过能打开对应文件）、已装配 37 个视频 / 612.1 秒、**0 个超出时长容差**（最差 1.0141）。覆盖面与 level-40 画面是剩下两块。

## 16. 2026-09-17：两条路各自的**前提条件**（实测出来的边界）

### 16.1 捕获兜底的前提：播放器必须在渲染
开了一次 5 分钟的捕获，**录到 0 帧**。查因：播放器进程还在（PID 31808），但 5 秒内 **0 次解码调用**——它已经停了/暂停了。capture_frames.py 挂在解码函数上，没有渲染就没有帧，这不是缺陷而是前提。

### 16.2 收钥匙的前提：那些分段必须落盘
把最新一次收钥匙的 **63 把钥匙**拿去映射：先在"最近 24 小时写入的文件"里找，再对**全部 11,788 个缓存文件**逐个试——**一把都对不上**。再查磁盘：最近 6 小时 D:\Downloads\EVPlayer2Downloads 之外也没有任何新 .ts。
结论：那次播放是**流式播放、没有落盘**，所以钥匙虽然拿到了，却没有对应的密文可用。

### 16.3 于是"任意一节课"有两个前提，工具都能自己检测
* 走解密路径：需要**该会话的段列表**（给 	k → 给钥匙）**且**段文件在磁盘上 → --watch 抓列表、map_keys.py 映射、key_inventory.py 逐把验证。
* 走捕获路径：需要**播放器正在渲染那一段**。
两条路各自的"没有产出"都能被工具明确报出来（"no new key could be placed"、"no frame came back"），而不是安静地交付一个空文件或灰画面。
