...
> **后续源码审计更正（同日，见 §14）：** 本节原先把反向的 64 字节 needle 命中误写成完整 NAL
> 逐字节相同。正向整包命中的证据保留，反向全量一致和 189,694 字节 IDR 全量一致尚需用修正后的工具重跑。
...
| 结论 | 实测 | 工具 |
| --- | --- | --- |
| 对课程 `119354`，捕获输入中有完整包与解密流逐字节相同 | 3,290 个可测包里有 1,270 个整包命中；反向每个 ≥ 256 B NAL 只验证了中间的 64 字节 | `verify_packet_identity.py` 的历史版本；§14 源码审计 |
...
**适用范围。** 整包逐字节相同的结论限于课程 `119354` 中正向命中的包；全量 NAL 一致需按 §14
重测。它也不是关于其他课的结论，其他课仍然是灰的（§13.7）。
...
**反向——历史运行只验证了每条 NAL 中间的 64 字节。** 9 条流中每条里所有至少 256 字节的 NAL，
其中间 needle 都在拼接的捕获包中找到。对 `119354-d3010742-508b-420b-8ba5-d7b351f02226`：

```
256-1k: 41/41   1k-4k: 5/5   4k-16k: 3/3   16k-64k: 1/1   64k+: 5/5
```

包括那个 189,694 字节 IDR 的中间 64 字节；这不是整个 IDR 相同的证据。

结论：对课程 `119354`，正向命中的 1,270 个整包与解密流相同。是否覆盖了全部大 NAL 和该 IDR，
需修正工具后重跑；暂不能据此排除所有解码器入口前的变换。
...
> ≥ 256 B NAL 的中间 needle。完整 NAL 覆盖率尚未重测；正向的计数依赖集合大小，比较时要拿同一口径的数来比。
...
两侧密钥相同且部分完整输入包命中（§13.2），支持已排除密钥推导错误；完整 NAL 覆盖尚待 §14 重测。
...
* **出画面** → 该原版构建可以解这个样本。继续复测其他灰课及完整课程，再决定导出流水线是否换解码器。
* **仍是平灰** → 排除原版 4.2.2 构建能直接解决这个样本；仍需区分播放器的 4.2.9 补丁、构建差异、
  上下文/side data 和内部变换。不能只凭这一次失败证明存在第二层。
...
## 14. 2026-09-16 接续：核对环境并修复字节验证判据

### 14.1 本机环境与未能运行的实验

本次工作目录是 `D:\codes\git\ev-reverse`，分支仍为 `feat/recover-segment-keys`，起点 `455fa64`。
本机找到 `D:\StudyApps\EVPlayer2\PlayerLibRender56_vs.dll`，SHA-256 与 §1 完全相同，
因此既有 RVA 可以复用。`D:\EVPlayer2_download` 中有 680 个文件，但未找到 §13 的
`d3010742` 样本、明文导出目录、`captured/pairing` 或 `captured/decode_frames`。目前没有运行中的播放器。
Python 是 `C:\Python313\python.exe`（3.13.7），PATH 中 FFmpeg 为 `N-120793-g4c3f94f265-20250819`，
并非 §13 实验使用的 8.1。

已重新下载 `imageio-ffmpeg==0.4.4` 的 Windows wheel，并提取、运行其 4.2.2 二进制。
下载及二进制放在被 Git 忽略的 `tools/parser-tools/captured/decoder-version/`，没有安装或替换产品解码器。
**灰样本上的 4.2.2 对照实验仍未运行，缺少明文样本。** 没有把本机密文直接交给 FFmpeg 当作这个实验。

### 14.2 源码发现与修复

历史版本 `verify_packet_identity.py` 正向会定位 needle，再比较整个包，确实是精确匹配。
反向却只做 `needle in haystack`，每条 NAL 只检查中间 64 字节，并把所有 <96 B NAL 自动记为命中。
而且旧 NAL 切分会把下一条四字节起始码的第一个零算入上一条 NAL。这些实现不能支撑
§13 原先的「每个大 NAL、包括整个 189,694 B IDR 均逐字节相同」。这是证据强度更正，不是已发现数据真的不同。

修正工具现在：

* 正确区分三/四字节 Annex B 起始码。
* 对每个 ≥256 B NAL 在单个捕获包内比较全部字节；needle 仅用于筛选候选。
* 分别报告 `identical`、`needle-only`、`absent`，逐条打印 IDR 的长度与结果；小 NAL 明确标为跳过。
* 不再拼接无关联捕获包作为命中依据，支持 `--pairing` 指向迁移后的数据目录。

新增 `test_verify_packet_identity.py` 的 7 个回归测试已通过，覆盖中间 needle 相同但头/尾被改动、
多个候选中后一个才是完整命中、跨包不能伪造完整匹配、三/四字节起始码和报告中的跳过项。
这些测试只验证工具判据，**不代表真实捕获已经通过完整比较**。没有捕获文件时也不能把 `absent` 解释成变换证据。

### 14.3 数据到位后直接执行

```powershell
python tools\parser-tools\verify_packet_identity.py --pairing '<真实 pairing 目录>'
$old = Join-Path $PWD 'tools\parser-tools\captured\decoder-version\imageio_ffmpeg\binaries\ffmpeg-win64-v4.2.2.exe'
python tools\parser-tools\decode_report.py '<真实明文灰文件.ts>' --ffmpeg $old --label ffmpeg-4.2.2
python tools\parser-tools\decode_report.py '<同一个明文灰文件.ts>' --label local-ffmpeg
```

先补齐完整 NAL 证据，再以同一文件的抽帧、亮度统计和解码错误做版本对照；出画面时还需亲眼检查抽出的帧。
若仍灰且完整输入已证实相同，下一步对比解码器内部拆包后的 NAL/RBSP、SPS/PPS、packet side data 和上下文。
本次静态核对到 `0xB9648` 将包数据传到 `0xB9AC4`，后者在 `0xB9BC0` 调用 `0x27150C`；
这只提供拆包路径的后续观察点，尚未证明该调用存在专有变换。

---

## 15. 2026-09-16 本机重建证据链：当前课程可解，尚未复现灰课

用户说明 §13 的样本来自另一台电脑，要求在本机重新进行实验。因此本节的数据与 §13 完全独立，
没有重测或替代另一台机器上的灰样本。

### 15.1 本机样本、字节及画面对照

在本机进程 23288 的一次真实播放中，`capture_local_decode.py` 同时捕获密钥、完整输入包和返回后的
AVFrame。有效第一轮目录是 `captured/local_decode/20260916-193552-049149/`：4,105 个输入包、
14 把不同密钥、10 张采样帧。用这些密钥在 `D:\EVPlayer2_download` 中定位、解密了 14 个连续分段。
第二轮目录是 `captured/local_decode/20260916-193920-919901/`，新增同会话的密钥推导前像、请求头和 API 明文捕获。
捕获文件及会话凭据全部位于已有 Git 忽略目录，未入库。

播放器帧是 3024×1968 coded、stride 3072、yuv420p；SPS 的可见画面为 **3024×1964**。
内容是 Vue/BPMN 的 VS Code 录屏，主题较暗，Y 均值约 49、标准差约 24。
这也说明 §13 的「真实画面均值约 220」只是那段亮色录屏的特征，不能作为通用成功阈值。

已运行的对照：

| 检查 | 本机结果 | 适用范围 |
| --- | --- | --- |
| 4.2.2 与本机新版全解码、抽帧 | 均零错误；n=0/60/200 的 Y mean/sd 分别为 48.9/24.08、49.0/23.99、49.0/24.07 | `119354-01cda443-3058-4801-86d1-8dcc13149c34.ts` |
| 修正后的完整 NAL 反向比较 | 176/176 个 ≥256 B NAL 整条相同，包括完整 **271,653 B IDR** | 同一分段，与第一轮完整输入包比较 |
| 第一轮 14 条流的扩大核对 | 1,949/1,978 个大 NAL 完整命中、needle-only=0、absent=29；正向 3,552 个整包命中 | 最后两个流缺少部分捕获，不能把 absent 当作变换证据 |
| 第二轮密钥推导与播放器密钥 | **14/14 相同**，extra 均为 `20220507` | 同一会话推导前像与 `hls_decode` 密钥 |
| 同一文件、同一实际输出 PTS 的帧 | 新版与 4.2.2 的可见 Y 平面均与播放器 **逐字节 100% 相同** | 第二轮 frame 8，AVFrame PTS=25,728,000，对应 `119354-b3568d01-f04f-4200-ae32-c9e9805ed794.ts` |
| 第一轮 14 个分段和导出 MP4 | 分段全解码均零错误；MP4 约 **133.391 秒、10,422,293 B**，全解码零错误 | `local-bpmn-fragment.mp4`，是课程连续片段，不是整课 |

同帧核对使用 **AVFrame 的 PTS**（本 DLL 中 `+0x88`），而非当前输入包 PTS，也不使用最近一次密钥设置
去猜帧归属。按分段 PTS 范围定位文件，以 `-copyts` 和 `settb=1/90000,select=eq(pts\,25728000)`
提取同一帧，再去掉播放器 stride padding 及 coded height 的底部填充行后比较完整 Y 平面。
两种 FFmpeg 都提取到了完整平面，退出码为 0，stderr 无错误。

### 15.2 本机 CLI 流水线与工具修正

本机原先的 `target/release/evmedia.exe` 是旧构建，甚至没有 `fetch` 子命令。
已用 `C:\Users\dd257\.cargo\bin\cargo.exe` 按当前源码完成 `cargo build --release -p evmedia`，
随后用第二轮捕获的有效会话执行本机 CLI 的 `fetch`，成功独立获取 5 个分段的清单。
其中 3 个密文已落盘，另 2 个尚未落盘；只取现存项生成清单，经 `derive → decode-ev → ffmpeg -c copy`
得到第二轮目录的 `local-cli-decoded.mp4`。这证明本机独立取清单及产品解密路径已跑通。

不要用旧研究工具 `fetch_list.py` 验证当前签名：它漏掉 `type=0&&ieway.cn@20200611`，
本次尝试得到「签名错误」。当前 Rust `api.rs` 包含正确签名前像，本机新构建已成功验证；未修改这段产品协议实现。

本次新增/必要修正：

* `capture_local_decode.py`：同一会话捕获密钥、完整包、实际输出 PTS、Y 平面、推导前像、请求头和解密后的 API 载荷；
  新目录保存证据，另写兼容 `build_videos.py` 的纯密钥日志。首次试跑误用了 Frida 只读的 `this.context`，
  已改为 `this.avctx`；`20260916-193519-964123` 目录的试跑没有完整输入包，不能用于输入字节核验。
* `decode_report.py`：由 ffprobe 读取实际宽高，直接统计 Y 平面；额外解码选项移至 `-i` 前，明确只测视频，
  选中帧只输出一张。这些改变使新结果与 §13 的 gray 全范围转换统计、混合音视频错误计数有所不同。
* `build_videos.py`：新增 `--downloads`，使本机下载路径可显式传入。

已验证完整 NAL 的 7 个回归测试；新旧 FFmpeg 对 320×240 的临时控制样本均正确输出第 0/2 帧，
第 20 帧正确报告不存在，`-threads 1` 也放在输入前。新增捕获和导出工具已做语法检查，Git diff 无空白错误。

### 15.3 仍待完成与本机继续命令

**本机当前这节课属于可解类，未复现灰课。** 上面的成功既不能证明 4.2.2 能解决 §13 的灰样本，
也不能证明播放器隐藏了第二层；暂不更换产品解码器。

进程 23288 在第二轮捕获后退出，原因未确认；一次新的内存扫描附加报 `ProcessNotRespondingError`，
没有得到扫描结果。已重新启动本机原播放器；截至本次记录，新进程 8444 尚无解码调用。
其截图为黑屏，但能读到课程列表的可访问性控件；刷新点击超时，键盘输入也未改变列表状态，
因此尚未自动打开另一节课。暂停在 x64dbg 系统断点的 18120 与这个新进程分开，不能把它当渲染进程。

在本机选择另一节已授权课程后，先挂捕获再播放（PID 以当时实测为准）：

```powershell
$env:PYTHONPATH = Join-Path $PWD 'tools\parser-tools\captured\runtime'
python tools\parser-tools\capture_local_decode.py --pid <当前渲染进程 PID> --seconds 180
python tools\parser-tools\build_videos.py --downloads D:\EVPlayer2_download `
    --keys '<该次捕获目录>\player_keys.jsonl' --out '<该次捕获目录>\export'
```

取得本机灰样本时，重跑 §14.3 的完整 NAL 核对和新旧版本对照，先证实失败出现在同一文件上，再往内部拆包路径走。

### 15.4 重启后实际播放的再次验证

用户确认已经播放后，重新附加实际渲染进程 8444，捕获目录为
`captured/local_decode/20260916-200415-916750/`。返回的 API 载荷中课程 UUID 仍为
`2786e755-a51b-4f9d-9fb8-3c981f796b7f`，输出帧仍是 Vue/BPMN 的 VS Code 录屏，
因此这是同一课程继续播放，不能记为另一节课或灰样本复现。

新捕获的 `119354-cb2bdb29-50af-450a-a171-8e1d4057dc41.ts`：

* 新版及 4.2.2 的全视频解码错误数均为 0；n=0/60/200 的 Y mean/sd 均为
  46.6/18.34、47.1/19.32、48.3/21.38。
* 162/162 条 ≥256 B NAL 完整命中播放器输入，needle-only=0、absent=0，
  包括完整 138,776 B IDR。此时仅提取了一条 H.264 流，正向比较中其他分段的包不覆盖，
  不能将其 absent 或 needle-only 解释为解密差异。
* frame 3 的实际 AVFrame PTS=12,960,000；去掉 stride 和底部填充后，可见 Y 平面
  共 5,939,136 B。新版与 4.2.2 均逐字节等于播放器，差异字节数为 0，退出码为 0，stderr 为空。
  结果保存于该目录 `paired-frame-report.json`。

这再次验证本机当前课程可解；§13 的灰课版本差异实验仍需另一节失败课程的本机样本。

本轮 180 秒捕获最终保存 5,398 个完整输入包、30 张输出帧及 18 把不同分段密钥。
18 个分段均在本机落盘并成功解密，最终导出
`export/01_cb2bdb29_143s-318s.mp4`，ffprobe 实测 **183.389333 秒、15,416,612 B**，
全解码错误数为 0。工具显示的 175.0 秒是首尾分段起始 PTS 的差，未包含最后一段时长，
因此以 ffprobe 实测为准。捕获完成前的 17 分段导出是中间结果，以这个 18 分段文件为准。

## 16. 2026-09-16 本机第二节课程：2-2 React 和 Vue 描述页面的区别

用户明确正在播放「2-2 React和Vue描述页面的区别」后，重新附加渲染进程 8444，
进行了 120 秒捕获，目录为 `captured/local_decode/20260916-202245-738535/`。
共保存 3,473 个完整输入包、30 张输出帧、12 把不同密钥。
四次解密后的 API 载荷均返回课程 UUID `fa6b2106-cdfc-44d2-aa83-b450006622e9`，
与 §15 的 Vue/BPMN 课程不同；直接查看返回帧 PNG，也确认了文档标题
「React 和 Vue 描述页面的区别」和渡一教育水印。

### 16.1 已运行的同分段及同帧对照

密钥定位并解密本机文件 `119354-8c0a269c-f090-4d0a-a730-62e08a22aa5c.ts`，
首 PTS=7,632,000，ffprobe 可见尺寸为 2880×1800。
播放器返回帧尺寸为 2880×1808、stride 2944，需要去掉底部填充及行尾 padding。

* 本机新版及 4.2.2 的全视频解码错误数均为 0；第 0/60/200 帧均为 Y mean=218.2、sd=41.09。
  该分段画面较静态，但不是纯灰；均值及标准差只是此段的测量值。
* 用实际 AVFrame PTS=7,785,000 精确提取 frame 7，同帧可见 Y 平面为 5,184,000 B。
  新版及 4.2.2 均与播放器逐字节完全相同，差异字节数为 0，退出码为 0，stderr 为空。
  此次同帧提取使用 `-threads 1`；全解码及第 0/60/200 帧对照使用默认线程设置。
  证据保存于 `paired-frame-report.json` 和对应原始 YUV 文件。
* 修正后的反向核对：29/29 条 ≥256 B NAL 完整命中播放器输入，needle-only=0、absent=0，
  包括完整 136,876 B IDR。正向只提供一条分段流，其他播放分段的包不在比较范围内，
  不能将其未命中解释为变换证据。
* 本轮 12 把密钥均定位到本机落盘分段，解密及导出成功。
  最终文件为 `export/01_8c0a269c_84s-193s.mp4`，ffprobe 实测 **116.732667 秒、5,923,759 B**，
  全视频解码错误数为 0；不是整课导出。

**当前捕获的这节课也可解，未复现 §13 的灰样本。** 两节本机课程成功不能证明旧版解码器
能解决灰课，也不能排除其他课程存在不同的码流或上下文要求；暂不更换产品解码器。

## 17. 2026-09-16 目标纠正：播放后导出整课，而非等待灰屏复现

用户明确要求「播放任意视频后，直接解密完整视频并合成 MP4/MKV」。此前 §15/16 的片段导出
没有满足这一目标，不再把寻找灰样本作为整课导出的前置条件。

### 17.1 补齐完整清单来源

用 `ReadProcessMemory` 读取本机进程 8444 的可读非模块内存，找到了 React/Vue 课程的完整
M3U8：191 个分段、`EXTINF` 总时长 **1908.899962 秒**、`#EXT-X-ENDLIST` 结束标记。
已捕获的 `8c0a269c` 和 `13a17388` 分段均在该清单中，上一节 BPMN 分段不在其中。
原始证据位于 Git 忽略目录 `captured/full-export-investigation/0417-1bcfbf99ac0.bin.m3u8`。

读取实际播放清单解决了 §7.4 中「完整名字来源不明」的问题。窗口请求中的 `idx=0..4`
是窗口相对编号；整课顺序必须来自原始 M3U8，不能靠捕获顺序或最高已见编号猜测。

### 17.2 通用监听和整课流水线

新增 `tools/export_video.py` 和 `tools/自动导出整课.cmd`。持续监听播放器签名前像与
授权请求；用当前窗口文件名绑定完整清单，再调用现有 Rust CLI 的
`fetch → download → derive → decode-ev`，保留音轨，合成 MP4/MKV。
启动时也尝试读取当前内存中的签名前像和 JWT，因此已经播放后启动工具也能处理。
多个候选不明确时等待实际请求确认，不能猜课。探针在挂钩前校验已验证 DLL 的 SHA256，
不同构建明确拒绝套用偏移。

工具要求服务器返回全部原始文件名与顺序，核对清单总时长，并对完整音视频进行 `-xerror`
解码校验。校验前输出 `.partial.mp4/.mkv`，通过后发布最终文件和 `report.json`。
会话、密钥、缓存和原始证据均不入库。面向用户的命令见 `docs/FULL-VIDEO-EXPORT.md`。

修复现有 Rust 下载器调度死锁：原来在把 future 入队时申请 semaphore permit，入队超过
并发数便等待尚未开始轮询的下载释放 permit。现在在 future 被轮询时申请 permit。
新增本地 HTTP 测试：5 段、并发 2，必须在超时前完成并保持全部文件字节。
本机已通过 9 项完整清单、归属、分批请求和发布重试测试，以及 Rust 的下载、API、清单共 16 项测试。

### 17.3 本机整课运行状态

React/Vue 的 191 项整课 `fetch` 已成功；176 个缓存分段复用，其余由独立下载补齐，
最终 `enc/` 与派生 manifest 均为 191 项。完整解密合并的 TS 为 **130,646,464 B**。
MP4 合成后 ffprobe 测得 **1908.923 秒、119,442,745 B**，含 H.264 视频及 AAC 音频，
画面为 2880×1800；MP4 已通过 FFmpeg `-xerror` 的完整音视频解码检查。
同一套整课数据又通过自动监听流程导出 MKV，ffprobe 测得 **1908.923 秒、118,326,704 B**，
同样含 H.264 视频及 AAC 音频。两种容器的 `report.json` 均记录为 `status=complete`。
